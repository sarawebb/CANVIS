...
for objid, ra, dec, field, ccd_num in zip(objects, ra_list, dec_list,
    ...
    path ='/fred/oz100/pipes/DWF_PIPE/MARY_WORK/'+ field + '_'+ run_date +'_mrt1_*/ccd'+ccd_num+'/images_resampled/sci_*.resamp.fits'
    ...
    for i, (key, (path, pixx, pixy)) in enumerate(mydic.items()):
        ...
        with fits.open(path) as hdu:
            ...
            #plt.show()
            plt.savefig(
                '/fred/oz100/CANVIS/cand_images/'+run+'/cand_' +
                format(objid, '05') + '_' + field + '_'+run+'/' + format(objid, '05') + '_' + run +
                '_cutout_' + format(i, '03') + '.png')

    path_vid = '/fred/oz100/CANVIS/cand_images/'+run+'/cand_{0:05d}_{1}_{2}/'.format(objid, field, run)
    filelist = []
    files = []
    for filename in os.listdir(path_vid):
        if filename.endswith('.png'):
            files.append(filename)
    writer = imageio.get_writer('/fred/oz100/CANVIS/cand_vids/'+run+'/cand_'+format(objid, '05')+'_'+run+'sci.mpeg', fps = 3)
    for im in files:
